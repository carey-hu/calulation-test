...
export const GAME_MODES = {
  ...
  decompAdd: {
    ...
    check: (v, t, inputStr, inputArray) => {
      if (!inputArray || inputArray.length < 3) {
        return { ok: false, display: `十位:${t.tens} 个位:${t.units} 总:${t.total}` };
      }
      const ok = parseInt(inputArray[0]) === t.tens
        && parseInt(inputArray[1]) === t.units
        && parseInt(inputArray[2]) === t.total;
      return { ok, display: `十:${t.tens} 个:${t.units} 总:${t.total}` };
    },
    gen: (n) => genN(n, () => {
      const a = randInt(10, 99);
      const b = randInt(10, 99);
  ...
  tripleDiv: {
    ...
    check: (v, t) => {
      if (Number.isInteger(t)) return { ok: v === t, display: t };
  ...
  carryJudge: {
    ...
    check: (v, t, inputStr, inputArray) => {
      if (!inputArray || inputArray.length < 2) {
        return { ok: false, display: t.replace(',', ' ') + ' 0' };
      }
  ...
  divScale: {
    ...
    check: (v, t, inputStr) => {
      ...
      const userVal = a / b;
      const ratio = userVal / t;
      const p10 = Math.round(Math.log10(ratio));
      const adjustedExact = t * Math.pow(10, p10);
...
export const MODE_GROUPS = {
  ...
  triple: {
    label: '三位数专项 (完整答案)',
    modes: ['carryJudge', 'borrowJudge', 'digitDetermine', 'triplePlus', 'tripleMinus', 'tripleAnyPlus', 'tripleAnyMinus', 'tripleMix', 'tripleMult', 'tripleDiv'],
  },
...
export const getModeConfig = (key) => GAME_MODES[key] || { name: key };

export const resolveActiveConfig = (modeKey, selectedDivisor) => {
...
export const getModeName = (modeKey, selectedDivisor) => {
